...
control.controller('ssCtrl', function($scope,$state,$ionicModal,$ionicLoading,$ionicScrollDelegate,$ionicPopup,$http,$ionicSideMenuDelegate,$ionicActionSheet){
  ...
  $scope.listViewClickFtn = function(){
	  ...
	  $ionicLoading.show({
                 ...
                 $http.post('http://build.myappbuilder.com/api/subscribers.json',formData11,{
                   ...
                   .success(function(response,status,headers,config){   
                   	...
                   	/*  $http({method: "POST", url:"http://build.myappbuilder.com/api/subscribers.json", cache: false, params:{"api_key":appKey,"subscriber[firstname]":$scope.fname,"subscriber[lastname]":$scope.lname,"subscriber[username]":$scope.email,"subscriber[email]":$scope.email, "subscriber[phone]":$scope.phone}})
                         .success(function(data, status){  */
        ...
        if(postmessage == ''){
           ...
           // window.wizSpinner.show(options);
            $.ajax({url:'http://build.myappbuilder.com/api/messages.json', type:"POST",data:{'message[body]':postmessage,'message[sender_id]':localStorage.sender_id,'api_key':appKey},
                   ...
                   error:function(){ $ionicLoading.hide(); alert("Failure");}
                   });
        }
    }else{
        //login();
